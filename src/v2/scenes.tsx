...
import { ArtworkCamera, getTargetHoldStartFrame, resolveArtworkFraming, targetNeedsTopText } from "./camera";
...
const Label: React.FC<{ children: string; revealStartFrame?: number }> = ({ children, revealStartFrame }) => (
  <EditorialText kind="label" maxLines={1} maxWidth={680} revealStartFrame={revealStartFrame} style={{ left: DESIGN.safe.left, position: "absolute", top: DESIGN.safe.top }}>
    {children}
  </EditorialText>
);
...
export const ArtworkOverviewScene: React.FC<SceneBase & { synthesis?: string }> = ({ artwork, durationInFrames, camera, synthesis }) => {
  ...
  return (
    <AbsoluteFill>
      <ArtworkCamera src={artwork.src} durationInFrames={durationInFrames} camera={camera ?? { move: "none", focalX: 0.5, focalY: 0.5, startScale: 1 }} artwork={artwork} contain darkBackdrop />
      {synthesis ? (
...
export const ArtworkDetailScene: React.FC<SceneBase & { detail: DetailPoint; observation?: string; label?: string }> = ({ artwork, durationInFrames, camera, detail, observation, label, debugTargetOverlay }) => {
  ...
  const revealStartFrame = getTargetHoldStartFrame(durationInFrames);
  ...
  return (
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ backgroundColor: "rgba(4, 10, 15, 0.2)" }} />
      {label ? <Label revealStartFrame={revealStartFrame}>{label}</Label> : null}
      <EditorialText kind="observation" maxLines={3} maxWidth={760} revealStartFrame={revealStartFrame} style={observationTextStyle(artwork, detail)}>
        {observation}
      </EditorialText>
    </AbsoluteFill>
  );
...
export const ObservationScene: React.FC<SceneBase & { observation?: string; label?: string; target: DetailPoint }> = ({ artwork, durationInFrames, camera, observation, label, target, debugTargetOverlay }) => {
  ...
  <AbsoluteFill>
    ...
    {label ? <Label revealStartFrame={revealStartFrame}>{label}</Label> : null}
    <EditorialText kind="observation" maxLines={3} maxWidth={760} revealStartFrame={revealStartFrame} style={observationTextStyle(artwork, target)}>
      {observation}
    </EditorialText>
  </AbsoluteFill>
  );
...
export const MetadataScene: React.FC<SceneBase> = ({ artwork, durationInFrames, camera }) => (
  <AbsoluteFill>
    <ArtworkCamera src={artwork.src} durationInFrames={durationInFrames} camera={camera ?? { move: "none", startScale: 1 }} artwork={artwork} contain darkBackdrop />
